...
export default {
  ...
  async mounted() {
    if (this.compName && this.demoName) {
      if (isDev) {
        this.sourceCode = (
          await import(/* @vite-ignore */ `../../examples/${this.compName}.vue?raw`)
        ).default;
      } else {
        this.sourceCode = await fetch(`${isDev ? '' : '/istar'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
      }
    }
    await this.$nextTick();
    Prism.highlightAll();
